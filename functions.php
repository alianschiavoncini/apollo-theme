...
function astrothemes_setup() {
	 ...
	 */
	load_theme_textdomain( 'apollo-hotel' );
	 ...
	 */
	//add_image_size( 'apollo-hotel-slideshow', 1920, 1080, true );
	add_image_size( 'apollo-hotel-slideshow', 1800, 800, true );
	add_image_size( 'apollo-hotel-slideshow-lg', 1400, 600, true );
	add_image_size( 'apollo-hotel-slideshow-md', 800, 600, true );
	add_image_size( 'apollo-hotel-slideshow-sm', 600, 600, true );
	add_image_size( 'apollo-hotel-featured-page', 800, 400, true );
	add_image_size( 'apollo-hotel-thumbnail', 600, 420, true );

	// This theme uses wp_nav_menu() in two locations.
	register_nav_menus( array(
		'primary' => esc_html__( 'Main Menu', 'apollo-hotel' )
	) );
...
function astrothemes_fonts_url() {
	 ...
	 */
	if ( 'off' !== _x( 'on', 'Lato font: on or off', 'apollo-hotel' ) ) {
...
function astrothemes_resource_hints( $urls, $relation_type ) {
	if ( wp_style_is( 'apollo-hotel-fonts', 'queue' ) && 'preconnect' === $relation_type ) {
		$urls[] = array(
...
function astrothemes_widgets_init() {
	...
	register_sidebar( array(
        'name' 			=> esc_html__( 'Main Sidebar', 'apollo-hotel' ),
        'id' 			=> 'sidebar',
        'description' 	=> esc_html__( 'Widgets in this area will be shown on all posts and pages.', 'apollo-hotel' ),
	...
	// footer col-1
	register_sidebar( array(
        'name' 			=> esc_html__( 'Footer: Column 1', 'apollo-hotel' ),
        'id' 			=> 'footer-col-1',
        'description' 	=> esc_html__( 'Widgets in this area will be shown on all posts and pages.', 'apollo-hotel' ),
        'before_widget' => '<div id="%1$s" class="widget %2$s">',
		'after_widget'  => '</div>',
	...
	// footer col-2
	register_sidebar( array(
        'name' 			=> esc_html__( 'Footer: Column 2', 'apollo-hotel' ),
        'id' 			=> 'footer-col-2',
        'description' 	=> esc_html__( 'Widgets in this area will be shown on all posts and pages.', 'apollo-hotel' ),
        'before_widget' => '<div id="%1$s" class="widget %2$s">',
		'after_widget'  => '</div>',
	...
	// footer col-3
	register_sidebar( array(
        'name' 			=> esc_html__( 'Footer: Column 3', 'apollo-hotel' ),
        'id' 			=> 'footer-col-3',
        'description' 	=> esc_html__( 'Widgets in this area will be shown on all posts and pages.', 'apollo-hotel' ),
        'before_widget' => '<div id="%1$s" class="widget %2$s">',
		'after_widget'  => '</div>',
	...
	// footer col-4
	register_sidebar( array(
        'name' 			=> esc_html__( 'Footer: Column 4', 'apollo-hotel' ),
        'id' 			=> 'footer-col-4',
        'description' 	=> esc_html__( 'Widgets in this area will be shown on all posts and pages.', 'apollo-hotel' ),
        'before_widget' => '<div id="%1$s" class="widget %2$s">',
		'after_widget'  => '</div>',
...
function astrothemes_custom_search_form( $form ) {
    ...
    <div>
	<!-- <label class="screen-reader-text" for="s">' . esc_html__( 'Search for:', 'apollo-hotel' ) . '</label> -->
    <input type="text" class="form-control" value="' . get_search_query() . '" name="s" id="s" />
    <input type="submit" id="searchsubmit" class="btn btn-primary btn-search" value="'. esc_html__( 'Search', 'apollo-hotel' ) .'" />
    </div>
    </form>';
...
function astrothemes_scripts() {
	...
	wp_enqueue_style( 'apollo-hotel-fonts', astrothemes_fonts_url(), array(), null );
	...
	// Theme stylesheet.
	wp_enqueue_style( 'apollo-hotel-css', get_stylesheet_uri(), null, $my_theme->get( 'Version' ) );
	
	$theme_css = get_template_directory_uri() . '/css/style.css';
	$responsive_theme_css = get_template_directory_uri() . '/css/responsive.css';
	wp_enqueue_style( 'apollo-hotel-css-style', $theme_css, null, $my_theme->get( 'Version' ) );
	wp_enqueue_style( 'apollo-hotel-responsive-css-style', $responsive_theme_css, null, time() );
...
function astrothemes_get_the_first_post_gallery( $post_id = false, $url_size = 'thumbnail', $link_url_size = 'large' ) {
	...
	if ( get_post_galleries( $post_id ) ) :
		...
		foreach( $srcs as $src ) :
			...
			$img = wp_get_attachment_image_src( $ids[$i], 'apollo-hotel-slideshow' );
			$img = $img[0];
			$img_lg = wp_get_attachment_image_src( $ids[$i], 'apollo-hotel-slideshow-lg' );
			$img_lg = $img_lg[0];
			$img_md = wp_get_attachment_image_src( $ids[$i], 'apollo-hotel-slideshow-md' );
			$img_md = $img_md[0];
			$img_sm = wp_get_attachment_image_src( $ids[$i], 'apollo-hotel-slideshow-sm' );
			$img_sm = $img_sm[0];
				...
				<div id="comment-<?php comment_ID(); ?>">
					...
					<div class="comment-author">
                        ...
                        <div class="comment-author-details">
                            <?php printf( '%s', sprintf( '<cite class="comment-author-name">%s</cite>', get_comment_author_link() ) ); ?>
                            <div class="comment-timestamp">
								<?php printf( esc_html__('%1$s at %2$s', 'apollo-hotel'), get_comment_date(), get_comment_time()); ?>
                            	<?php edit_comment_link( esc_html__( 'Edit', 'apollo-hotel' ), ' - ' ); ?>
                            </div>
                            <div class="btn-reply">
					...
					<div class="comment-body">
						...
						<p class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'apollo-hotel' ); ?></p>
						<?php endif; ?>
...
?>
			<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
				<p><?php esc_html_e( 'Pingback:', 'apollo-hotel' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( esc_html__( '(Edit)', 'apollo-hotel' ), ' ' ); ?></p>
			</li>
<?php
		break;
...
function astrothemes_list_child_pages() { 
	...
	if ( $childpages ) {
		...
		$str .= '<h2>' . esc_html__( 'Related Pages', 'apollo-hotel' ) . '</h2>';
